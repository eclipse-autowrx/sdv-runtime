...
"""
Universal lite version of syncer that works with either pexpect or async subprocess.
Automatically chooses the best available option for subprocess management.
"""
...
# Try to import pexpect first, fallback to async subprocess
try:
    from pexpect_util import pexpect_subpiper
    USE_PEXPECT = True
    print("Using pexpect for subprocess management", flush=True)
except ImportError:
    try:
        from async_subprocess_util import async_subprocess_subpiper
        USE_PEXPECT = False
        print("Using async subprocess for subprocess management", flush=True)
    except ImportError:
        print("ERROR: Neither pexpect nor async_subprocess_util available!", flush=True)
        print("Please install pexpect: pip install pexpect", flush=True)
        sys.exit(1)

# Configuration
DEFAULT_KIT_SERVER = 'https://kit.digitalauto.tech'
DEFAULT_RUNTIME_NAME = 'UniversalLiteRuntime'
BORKER_IP = '127.0.0.1'
BROKER_PORT = 55555
...
async def handle_run_python_app(data):
    ...
    # Start process using the available subprocess method
    try:
        if USE_PEXPECT:
        ...
        lsOfRunner.append({
            ...
            "from": time.time(),
            "method": "pexpect" if USE_PEXPECT else "async_subprocess"
        })
...
async def handle_get_runtime_info(data):
    ...
    for runner in lsOfRunner:
        runner_info.append({
            ...
            "method": runner.get("method", "unknown")
        })
    ...
    await sio.emit("messageToKit-kitReply", {
        ...
        "data": {
            ...
            "subprocess_method": "pexpect" if USE_PEXPECT else "async_subprocess"
        }
    })
...
async def main():
    ...
    CLIENT_ID = "UniversalLiteRuntime-" + os.getenv('RUNTIME_NAME', DEFAULT_RUNTIME_NAME)
    
    print(f"Universal Lite Runtime starting with ID: {CLIENT_ID}", flush=True)
    print(f"Connecting to server: {server_url}", flush=True)
    print(f"Using subprocess method: {'pexpect' if USE_PEXPECT else 'async_subprocess'}", flush=True)
    ...
    try:
        ...
        cleanup_task.cancel()
        print("Universal Lite Runtime stopped", flush=True)
