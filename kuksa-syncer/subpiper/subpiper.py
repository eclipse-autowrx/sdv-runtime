...
def subpiper(
    ...
    silent: bool = False,
) -> Optional[Tuple[subprocess.Popen, int, List[str], List[str]]]:
...
class _SubPiper:
    ...
    def execute(self) -> Optional[Tuple[subprocess.Popen, int, List[str], List[str]]]:
        # add user path
        ...
        if self.finished_callback is None:
            ...
            return self.proc, retcode, self._stdout_buffer, self._stderr_buffer
        else:
    ...
    def _handle_lines(self):
        ...
        if oline:
            ...
            if self.stdout_callback is not None:
                self.stdout_callback(self.master_id, oline)
            else:
                if not self.silent:
        ...
        if eline:
            ...
            if self.stderr_callback is not None:
                self.stderr_callback(self.master_id, eline)
            else:
                if not self.silent:
    ...
    def _wait_for_process(self) -> int:
        ...
        if self.finished_callback is not None:
            self.finished_callback(self.master_id, retcode)
